...
module.exports = {
        ...
        .then((pila) => {
          ...
          if (currRepo == undefined) {
            this.createRepo(pila, name, path, (repo) => {
              // Create audios.
              this.getAudios(pila, repo, () => {
                resolve();
              })
            });
          } else {
            // Create audios.
            this.getAudios(pila, currRepo, () => {
              resolve();
            })
          }
  ...
  getAudios: function(pila, repo, callback) {
    ...
    this.getLocalFiles(repo.get('path'), (error, files) => {
      ...
      if (files.length === 0) {
        ...
        files.forEach((file) => {
          this.createAudio(file, pila, repo, (audio) => {
  ...
  createAudio: function(file, pila, repo, callback) {
    var audio = {};
    ...
    if (fileDeets) {
      audio.duration = fileDeets.format.duration;
    }
